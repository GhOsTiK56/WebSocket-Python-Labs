...
def connect(sid, environ):
    ...
    '''
    функция формирует пакет Socket.IO с типом EVENT.
    Упаковывает туда имя события (event_name) и полезные данные (data).
    Отправляет этот пакет по WebSocket (или long-polling) выбранным клиентам.
    '''
